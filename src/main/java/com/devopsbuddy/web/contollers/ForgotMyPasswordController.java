...
public class ForgotMyPasswordController {
    ...
    public static final String FORGOT_PASWORD_URL_MAPPING = "/forgotmypassword";
    ...
    @RequestMapping(value = FORGOT_PASWORD_URL_MAPPING, method = RequestMethod.GET)
    public String forgotPasswordGet() {

        return EMAIL_ADDRESS_VIEW_NAME;
    }

    @RequestMapping(value = FORGOT_PASWORD_URL_MAPPING, method = RequestMethod.POST)
    public String forgotPasswordPost(HttpServletRequest request,
        ...
        if (null == passwordResetToken) {
            LOG.warn("Couldnt find a password reset token for email {}", email);
        } else {
    ...
    public String changeUserPasswordGet(@RequestParam("id") long id,
        ...
        User user = passwordResetToken.getUser();
        if (user.getId() != id) {
            LOG.error("The user id {} paramtereyle gelen, tokenle {} anlaşmış olan id'ye {} eşleşmiyor!", id, user.getId(), token);
            model.addAttribute(PASSWORD_RESET_ATTRIBUTE_NAME, "false");
    ...
    public String changeUserPasswordPost(@RequestParam("principal_id") long userId,
        ...
        if (null == authentication) {
            LOG.error("An unatuh user tried to invoke the reset password POST metgod");
            model.addAttribute(PASSWORD_RESET_ATTRIBUTE_NAME, "false");
